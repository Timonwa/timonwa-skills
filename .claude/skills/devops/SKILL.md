...
- **One composite setup action** (`.github/actions/setup`): `corepack enable` → `pnpm` + Node (`cache: pnpm`) → `pnpm install --frozen-lockfile` → turbo remote-cache env. Every job reuses it; never re-paste those steps.
- **`actions/checkout` stays in the workflow, never inside the composite action.** A local composite action is read from the checked-out workspace, so the runner cannot find `.github/actions/setup` until checkout has already run — putting checkout inside it fails with `Can't find 'action.yml' … Did you forget to run actions/checkout?`. Each job therefore does checkout, then `uses: ./.github/actions/setup`. Set `fetch-depth: 0` on the checkout wherever history matters: `turbo --affected` diffs against a base, and changesets works out what was already released.
- **`pnpm/action-setup` must come before `actions/setup-node`.** The Node step's `cache: pnpm` resolves the pnpm store path, and it cannot find a store that does not exist yet.
