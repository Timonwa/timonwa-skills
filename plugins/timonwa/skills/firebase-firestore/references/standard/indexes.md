...
In Standard Edition, Firestore **automatically creates** a single-field index for every field in a document (and subfields in maps). * **Support**: Simple equality queries (`==`) and single-field range/sort queries (`<`, `<=`, `orderBy`). * **Behavior**: You generally don't need to manage these unless you want to _exempt_ a field.
...
Deploy indexes only: `bash npx -y firebase-tools@latest deploy --only firestore:indexes`
