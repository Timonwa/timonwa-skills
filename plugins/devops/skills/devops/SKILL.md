...
- **"Secret" vs "external identifier" are different categories** — non-secret-but-required-at-boot values (project ids, cloud names, public keys) belong in env; **derivable per-tier values** (base URLs, cookie domain, CORS origins, allowlists) belong in code, in a `runtime.config` keyed off `APP_ENV`, **not** env vars. The decision gate: does it differ per env AND is it not derivable? Then it's env.
- **Public `NEXT_PUBLIC_*` vs server-only.** **Tier is `APP_ENV`, not `NODE_ENV`.**
- **`.env.example` tracks every required var**; `.env.local` is gitignored. **Secrets via Cloud Secret Manager** (→ `firebase`), never committed or `NEXT_PUBLIC_`-exposed.
