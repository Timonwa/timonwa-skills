...
1. **Add Marketplace**: Run the following command to add the marketplace (this uses the default User scope):
   ```bash
...
1. **Install Plugins**: Run the following command to install the plugin:
   ...
   ```
1. **Verify Installation**: Re-run the checks in steps 1 and 2 to confirm the skills and the MCP server are now available.
