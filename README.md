...
| [api-architecture](.claude/skills/api-architecture)                               | skill   | Route→service→data layering, `.pick()` write schemas, RBAC guards, rate-limit tiers       |                                                         |
...
| [logo-generation](.claude/skills/logo-generation)                                 | skill   | A brand's logo, icon and favicon set, with the wordmark outlined so it needs no font      |                                                         |
